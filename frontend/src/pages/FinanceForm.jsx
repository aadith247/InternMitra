...
export default function FinanceForm({ embedded = false }) {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        const { data } = await axios.get(`${API_BASE}/students/profile/finance`, { headers })
        const existing = data?.data || null
        if (existing) {
          setForm(prev => ({
            ...prev,
            ...existing,
            certifications: Array.isArray(existing.certifications) && existing.certifications.length ? existing.certifications : prev.certifications,
            courses: Array.isArray(existing.courses) && existing.courses.length ? existing.courses : prev.courses,
  ...
  const onSubmit = async (e) => {
    ...
    try {
      const jwt = localStorage.getItem('token')
      const headers = jwt ? { Authorization: `Bearer ${jwt}` } : {}
      await axios.post(`${API_BASE}/students/profile/finance`, form, { headers })
      setMsg('Saved! Your finance profile has been updated.')
    } catch (e) {
  ...
  const content = (
    <>
      {!embedded && <h1 className="text-2xl font-bold mb-4">Finance Profile</h1>}
      ...
      <form onSubmit={onSubmit} className="space-y-8">
          ...
          <div className="bg-white p-6 rounded border">
            ...
            {form.certifications.map((row, idx) => (
              <div key={idx} className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-3">
                <input className="border rounded p-2" placeholder="Certification (e.g., CFA L1)" value={row.name} onChange={e => updateArrayField('certifications', idx, 'name', e.target.value)} />
                <input className="border rounded p-2" placeholder="ID/Reg No (optional)" value={row.id} onChange={e => updateArrayField('certifications', idx, 'id', e.target.value)} />
                <input className="border rounded p-2" placeholder="Issuer" value={row.issuer} onChange={e => updateArrayField('certifications', idx, 'issuer', e.target.value)} />
                <div className="md:col-span-3 text-right"><button type="button" className="text-sm text-red-600" onClick={() => removeRow('certifications', idx)}>Remove</button></div>
              </div>
            ))}
            <Button type="button" variant="outline" onClick={() => addRow('certifications', { name: '', id: '', issuer: '' })}>Add Certification</Button>
          </div>
          ...
          <div className="bg-white p-6 rounded border">
            ...
            {form.courses.map((row, idx) => (
              <div key={idx} className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-3">
                <input className="border rounded p-2" placeholder="Course Name" value={row.name} onChange={e => updateArrayField('courses', idx, 'name', e.target.value)} />
                <input className="border rounded p-2" placeholder="Provider (e.g., Coursera)" value={row.provider} onChange={e => updateArrayField('courses', idx, 'provider', e.target.value)} />
                <div className="md:col-span-2 text-right"><button type="button" className="text-sm text-red-600" onClick={() => removeRow('courses', idx)}>Remove</button></div>
              </div>
            ))}
            <Button type="button" variant="outline" onClick={() => addRow('courses', { name: '', provider: '' })}>Add Course</Button>
          </div>

          <div className="bg-white p-6 rounded border">
            <h2 className="font-semibold mb-3">Projects / Case Studies</h2>
            {form.projects.map((row, idx) => (
              <div key={idx} className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-3">
                ...
                <input className="border rounded p-2" placeholder="Link" value={row.link} onChange={e => updateArrayField('projects', idx, 'link', e.target.value)} />
                ...
                <div className="md:col-span-3 text-right"><button type="button" className="text-sm text-red-600" onClick={() => removeRow('projects', idx)}>Remove</button></div>
              </div>
            ))}
            <Button type="button" variant="outline" onClick={() => addRow('projects', { name: '', link: '', desc: '' })}>Add Project</Button>
          </div>
          ...
          <div className="bg-white p-6 rounded border">
            ...
            <input className="border rounded p-2 w-full" placeholder="Tools (comma separated)" value={form.tools} onChange={e => setForm({ ...form, tools: e.target.value })} />
          </div>

          <Button type="submit" disabled={saving}>{saving ? 'Saving...' : 'Save Finance Profile'}</Button>
      </form>
    </>
  )
